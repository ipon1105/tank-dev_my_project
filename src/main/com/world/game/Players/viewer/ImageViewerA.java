...
public class ImageViewerA  extends ImageViewer{
    ...
    public ImageViewerA(float scale) {
        super(scale);

        for (int i = 0; i < 32; i++)
            Display.textureBuffer.resize_index(Texture.gUP_1.ordinal() + i,
                    (int) ((int)Display.textureBuffer.get(Texture.fromOrdinal(Texture.gUP_1.ordinal() + i)).getWidth() * 0.65),
                    (int) ((int)Display.textureBuffer.get(Texture.fromOrdinal(Texture.gUP_1.ordinal() + i)).getWidth() * 0.65));

        for (int i = 0; i < 32; i++)
            Tank[(int) Math.ceil(i/8)][i%8] = Display.textureBuffer.get(Texture.fromOrdinal(Texture.gUP_1.ordinal() + i));
